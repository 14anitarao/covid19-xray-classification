...
from model import AdemNet, AdemNetV2, AdemNetV3, get_extractor, AdemNetV4
...
if __name__ == '__main__':
    ...
    model.fit(
        image_gen.flow(X_train, encode(y_train), batch_size=BATCH_SIZE),
        epochs=EPOCHS,
        validation_data=(X_test, encode(y_test)),
        callbacks=[
    ...
    test_scores['f1_nofin'], test_scores['f1_covid'], test_scores['f1_pneu'] = \
        metrics.f1_score(y_test, y_pred, average=None)
    test_scores['precision_nofin'], test_scores['precision_covid'], test_scores['precision_pneu'] = \
        metrics.precision_score(y_test, y_pred, average=None)
    test_scores['recall_nofin'], test_scores['recall_covid'], test_scores['recall_pneu'] = \
        metrics.recall_score(y_test, y_pred, average=None)
    test_scores['f1'] = metrics.f1_score(y_test, y_pred, average='weighted')
    test_scores['precision'] = \
        metrics.precision_score(y_test, y_pred, average='weighted')
    test_scores['recall'] = metrics.recall_score(y_test, y_pred, average='weighted')
    test_scores['auc'] = metrics.roc_auc_score(encode(y_test), y_proba)

    train_scores = {}
    train_scores['accuracy'] = metrics.accuracy_score(y_train, t_pred)
    train_scores['f1_nofin'], train_scores['f1_covid'], train_scores['f1_pneu'] = \
        metrics.f1_score(y_train, t_pred, average=None)
    train_scores['precision_nofin'], train_scores['precision_covid'], train_scores['precision_pneu'] = \
        metrics.precision_score(y_train, t_pred, average=None)
    train_scores['recall_nofin'], train_scores['recall_covid'], train_scores['recall_pneu'] = \
        metrics.recall_score(y_train, t_pred, average=None)
    train_scores['f1'] = metrics.f1_score(y_train, t_pred, average='weighted')
    train_scores['precision'] = \
        metrics.precision_score(y_train, t_pred, average='weighted')
    train_scores['recall'] = metrics.recall_score(y_train, t_pred, average='weighted')
    train_scores['auc'] = metrics.roc_auc_score(encode(y_train), t_proba)

    print(cm_test)
    now = datetime.now().strftime('%Y%m%d%H%M')

    cm_test.to_csv(f'results/{ARCH}_cm_test_{now}.csv')
    cm_train.to_csv(f'results/{ARCH}_cm_train_{now}.csv')
    with open(f"results/{ARCH}_test_scores_{now}.json", "w") as f:
        json.dump(test_scores, f)
    with open(f"results/{ARCH}_train_scores_{now}.json", "w") as f:
        json.dump(train_scores, f)
    with open(f"filenames/filenames_train_{now}.json", "w") as f:
        json.dump(names_train, f)
    with open(f"filenames/filenames_test_{now}.json", "w") as f:
    ...
    e_train.to_csv(f"sets/train_{now}.csv")
    e_test.to_csv(f"sets/test_{now}.csv")
